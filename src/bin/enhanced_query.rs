...
use local_doc_assistant::{OllamaClient, PromptBuilder, DocumentIndex, RAGValidator, OllamaOptions};
...
async fn main() -> Result<()> {
    ...
    let default_extensions = vec!["c".to_string(), "h".to_string(), "lpc".to_string(), "txt".to_string(), "md".to_string()];
    let mut doc_index = DocumentIndex::new(mud_references_path.clone(), default_extensions.clone());
    println!("Building index (this may take a moment)...");
    let indexed_files = doc_index.build_index()?;
    println!("  ✓ Indexed {} files", indexed_files);
    
    let validator = RAGValidator::new(doc_index.clone(), mud_references_path.clone())?;
    ...
    let prompt_builder = PromptBuilder::new(templates_path)
        ...
        .with_reference_index(doc_index);
        ...
        if !validation.retrieved_documents.is_empty() {
            ...
            for (i, doc) in validation.retrieved_documents.iter().take(5).enumerate() {
                let status_icon = match doc.validation_status {
                    local_doc_assistant::rag_validator::ValidationStatus::Verified => "✓",
                    local_doc_assistant::rag_validator::ValidationStatus::CrossReferenced => "⊕",
                    local_doc_assistant::rag_validator::ValidationStatus::SingleSource => "○",
                    local_doc_assistant::rag_validator::ValidationStatus::Uncertain => "?",
                };
