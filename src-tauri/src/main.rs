...
use local_doc_assistant::{OllamaClient, ContextManager, PromptBuilder, DocumentIndex, UserProjectConfig, load_user_config, save_user_config};
...
async fn ask_ollama(
    ...
    let options = Some(local_doc_assistant::ollama_client::OllamaOptions::with_defaults(
        temperature,
        top_p,
        top_k,
        num_predict,
...
fn ask_ollama_stream(
    ...
    tauri::async_runtime::spawn(async move {
        let mut stream = client.generate_stream_with_cancel(
            &model,
            &prompt,
            Some(local_doc_assistant::ollama_client::OllamaOptions::with_defaults(
                temperature,
                top_p,
                top_k,
                num_predict,
...
fn analyze_driver(path: String) -> Result<serde_json::Value, String> {
    local_doc_assistant::driver_analyzer::efuns_json(&path)
...
#[tauri::command]
async fn configure_user_project(
    name: String,
    directory: String,
    ...
    let config = UserProjectConfig {
        ...
        reference_directory: PathBuf::from(directory),
        file_extensions: extensions.clone(),
    ...
    Ok(format!("Configuration saved for project: {}", name))
}

#[tauri::command]
async fn get_user_config() -> Result<UserProjectConfig, String> {
    load_user_config()
        .map_err(|e| e.to_string())
}

#[tauri::command]
fn browse_project_directory() -> Result<Option<String>, String> {
...
async fn rebuild_document_index(
    ...
    *index = DocumentIndex::new(
    ...
    match index.build_index() {
        Ok(count) => Ok(format!("Successfully indexed {} files", count)),
        Err(e) => Err(format!("Failed to build index: {}", e)),
    }
}

#[tauri::command]
fn get_common_file_extensions() -> Result<Vec<serde_json::Value>, String> {
    Ok(vec![
        serde_json::json!({"name": "Rust", "extensions": ["rs", "toml"]}),
        serde_json::json!({"name": "Python", "extensions": ["py", "pyx", "pyi"]}),
        serde_json::json!({"name": "JavaScript/TypeScript", "extensions": ["js", "ts", "jsx", "tsx", "json"]}),
        serde_json::json!({"name": "C/C++", "extensions": ["c", "h", "cpp", "hpp", "cc"]}),
        serde_json::json!({"name": "Java", "extensions": ["java"]}),
        serde_json::json!({"name": "Go", "extensions": ["go"]}),
        serde_json::json!({"name": "Documentation", "extensions": ["md", "txt", "rst"]}),
        serde_json::json!({"name": "Web", "extensions": ["html", "css", "scss"]}),
        serde_json::json!({"name": "Config", "extensions": ["yaml", "yml", "toml", "ini", "conf"]}),
        serde_json::json!({"name": "LPC (Legacy)", "extensions": ["lpc", "c", "h"]}),
    ])
}
...
fn main() {
    ...
    eprintln!("Initializing DocumentIndex...");
    eprintln!("Index dir: {:?}", index_dir);
    eprintln!("Corpus root: {:?}", corpus_root);

    let default_extensions = vec!["c".to_string(), "h".to_string(), "lpc".to_string(), "y".to_string(), "txt".to_string(), "md".to_string(), "json".to_string(), "jsonl".to_string()];

    let mut doc_index = match DocumentIndex::open_or_build(&index_dir, &corpus_root, default_extensions.clone()) {
        Ok(idx) => {
            eprintln!("DocumentIndex initialized successfully");
            idx
        }
        Err(e) => {
            eprintln!("Warning: DocumentIndex init failed: {}, using empty index", e);
            DocumentIndex::new(corpus_root.clone(), default_extensions)
        }
    };

    if corpus_root.exists() {
        match doc_index.build_index() {
    ...
    let state = AppState {
        ...
        index: Arc::new(RwLock::new(doc_index)),
