...
fn test_mud_reference_index_creation() {
    ...
    // Create index
    let default_extensions = vec!["c".to_string(), "h".to_string(), "lpc".to_string()];
    let index = DocumentIndex::open_or_build(&temp_dir, &corpus_dir, default_extensions);
    assert!(index.is_ok(), "Failed to create DocumentIndex");
...
fn test_search_functionality() {
    ...
    let default_extensions = vec!["c".to_string(), "h".to_string(), "lpc".to_string()];
    let mut index = DocumentIndex::open_or_build(&temp_dir, &corpus_dir, default_extensions)
